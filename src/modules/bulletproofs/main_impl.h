...
int secp256k1_bulletproof_rangeproof_prove(
    ...
    for (i = 0; i < n_commits; i++) {
        ...
        if (commits == NULL) {
            /* Calculate commitment from blinding factor. The libwally Pedersen
             * helper hard-codes the blinding generator to G via the precomputed
             * ecmult_gen table, so callers must construct generators with
             * secp256k1_generator_const_g as the blinding generator. */
            secp256k1_gej commitj;
            secp256k1_pedersen_ecmult(&ctx->ecmult_gen_ctx, &commitj, &blinds[i], value[i], &value_genp);
    ...
    if (t_one != NULL) {
        tge = (secp256k1_ge *)secp256k1_scratch_alloc(&ctx->error_callback, scratch, 2 * sizeof(secp256k1_ge));
        if (tge == NULL) {
            secp256k1_scratch_apply_checkpoint(&ctx->error_callback, scratch, scratch_checkpoint);
            return 0;
        }
